...
async function go(page) {
  ...
  try {
    ...
    if (page === 'photo-sync') return renderPhotoSync();
...
// ---------------------------------------------------------------------------
// Sincronização segura pelo navegador -> pasta local escolhida pelo Administrador
// ---------------------------------------------------------------------------
const SYNC_DB='rota-proxima-local-sync';
const SYNC_STORE='handles';
const SYNC_HANDLE_KEY='rota-pev-directory';
const monthNames=['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'];
const evidenceFolder={collection_material:'Coleta',delivery_drum_location:'Entrega',weighing_scale:'Pesagem'};
const evidencePrefix={collection_material:'material',delivery_drum_location:'tambor-local',weighing_scale:'balanca'};

function fsApiAvailable(){ return typeof window.showDirectoryPicker === 'function' && 'indexedDB' in window; }
function cleanFsName(v=''){
  let s=String(v||'').replace(/[<>:"/\\|?*\x00-\x1F]/g,' ').replace(/\s+/g,' ').trim().replace(/[. ]+$/,'');
  if(!s) s='Sem nome';
  if(/^(con|prn|aux|nul|com[1-9]|lpt[1-9])$/i.test(s)) s=`_${s}`;
  return s.slice(0,110);
}
function fileExt(path=''){
  const m=String(path).match(/\.([a-zA-Z0-9]{2,5})$/); return m?m[1].toLowerCase():'jpg';
}
function evidenceFilename(item){
  ...
  let prefix=evidencePrefix[item.evidence_type]||'foto';
  if(item.evidence_type==='weighing_scale' && item.weight_kg!=null){
    const kg=Number(item.weight_kg).toFixed(3).replace(/0+$/,'').replace(/\.$/,'').replace('.', '-');
    prefix+=`_${kg}kg`;
  }
  return `${prefix}_${stamp}_e${item.id}.${fileExt(item.storage_path)}`;
}
function evidencePathParts(item){
  const baseDate=item.route_date?new Date(`${item.route_date}T12:00:00`):(item.created_at?new Date(item.created_at):new Date());
  const year=String(baseDate.getFullYear());
  const month=`${String(baseDate.getMonth()+1).padStart(2,'0')} - ${monthNames[baseDate.getMonth()]}`;
  const route=`Rota ${String(item.route_id||0).padStart(5,'0')} - ${cleanFsName(item.route_name||'Rota')}`;
  const pev=cleanFsName(item.pev_name||`PEV ${item.pev_id||''}`);
  const type=evidenceFolder[item.evidence_type]||'Fotos';
  return [year,month,route,pev,type];
}
function idbOpen(){
...
async function idbSetHandle(handle){ const db=await idbOpen(); return new Promise((resolve,reject)=>{const tx=db.transaction(SYNC_STORE,'readwrite');tx.objectStore(SYNC_STORE).put(handle,SYNC_HANDLE_KEY);tx.oncomplete=()=>{db.close();resolve();};tx.onerror=()=>{db.close();reject(tx.error);};}); }
async function idbGetHandle(){ const db=await idbOpen(); return new Promise((resolve,reject)=>{const tx=db.transaction(SYNC_STORE,'readonly');const r=tx.objectStore(SYNC_STORE).get(SYNC_HANDLE_KEY);r.onsuccess=()=>{db.close();resolve(r.result||null);};r.onerror=()=>{db.close();reject(r.error);};}); }
async function handlePermission(handle,ask=false){
...
async function restoreLocalSyncHandle(){
  ...
  try{
    ...
    if(!h)return;
    state.localSyncHandle=h;
    if(await handlePermission(h,false)) startLocalSyncTimer();
  }catch(e){console.warn('restore sync handle',e);}
}
async function chooseLocalSyncFolder(){
  if(!fsApiAvailable())throw new Error('Seu navegador não permite gravar diretamente em uma pasta. Use Chrome ou Edge atualizado pelo endereço HTTPS do Rota Próxima.');
  const h=await window.showDirectoryPicker({id:'rota-proxima-rota-pev',mode:'readwrite'});
  if(h.name!=='Rota - PEV' && !confirm(`A pasta escolhida foi "${h.name}".\n\nO destino esperado é "Rota - PEV". Deseja usar esta pasta mesmo assim?`))return null;
  if(!(await handlePermission(h,true)))throw new Error('Permissão de gravação na pasta não concedida.');
  state.localSyncHandle=h; await idbSetHandle(h); startLocalSyncTimer(); return h;
}
...
async function fileAlreadyExists(dir,name){try{await dir.getFileHandle(name,{create:false});return true;}catch{return false;}}
async function syncEvidenceFiles(interactive=true){
  if(state.localSyncRunning)return {busy:true};
  state.localSyncRunning=true; updatePhotoSyncStatus('Sincronizando...');
  try{
    let h=state.localSyncHandle;
    if(!h){ if(!interactive) return {needsPermission:true}; h=await chooseLocalSyncFolder(); if(!h)return {cancelled:true}; }
    if(!(await handlePermission(h,interactive))){ updatePhotoSyncStatus('Clique em “Autorizar pasta” para permitir a gravação.','warn'); return {needsPermission:true}; }
    const data=await api('/api/evidence-sync'); const items=data.items||[];
    let copied=0,existing=0,errors=[];
    for(const item of items){
      try{
        const dir=await ensureDir(h,evidencePathParts(item)); const name=evidenceFilename(item);
        if(await fileAlreadyExists(dir,name)){existing++;continue;}
        const res=await fetch(`/api/evidence-sync/file?id=${encodeURIComponent(item.id)}`,{credentials:'same-origin'});
        if(!res.ok){let msg=`Erro ${res.status}`;try{msg=(await res.json()).error||msg;}catch{}throw new Error(msg);}
        const blob=await res.blob(); const fh=await dir.getFileHandle(name,{create:true}); const wr=await fh.createWritable(); await wr.write(blob); await wr.close(); copied++;
      }catch(e){errors.push(`#${item.id}: ${e.message}`);}
    }
    state.localSyncLast=new Date();
    const result={total:items.length,copied,existing,errors};
    if(interactive){
      if(errors.length)toast(`${copied} nova(s) foto(s) salva(s); ${errors.length} erro(s).`,'error');
      else toast(copied?`${copied} nova(s) foto(s) salva(s) em ${h.name}.`:'Nenhuma foto nova para copiar.','success');
    }
    updatePhotoSyncStatus(errors.length?`${copied} copiadas • ${existing} já existentes • ${errors.length} erros`:`${copied} copiadas • ${existing} já existentes` ,errors.length?'warn':'ok');
    return result;
  } finally {state.localSyncRunning=false;}
}
function updatePhotoSyncStatus(text,kind=''){
  const el=$('#photoSyncLiveStatus'); if(!el)return; el.textContent=text; el.className=`sync-status ${kind}`;
  const folder=$('#photoSyncFolder'); if(folder)folder.textContent=state.localSyncHandle?state.localSyncHandle.name:'Nenhuma pasta autorizada';
  const last=$('#photoSyncLast'); if(last)last.textContent=state.localSyncLast?state.localSyncLast.toLocaleString('pt-BR'):'Ainda não executada';
}
async function renderPhotoSync(){
  if(state.user.role!=='admin')throw new Error('Sem permissão');
  const supported=fsApiAvailable();
  if(!state.localSyncHandle && supported){try{state.localSyncHandle=await idbGetHandle();}catch{}}
  let granted=false;if(state.localSyncHandle)try{granted=await handlePermission(state.localSyncHandle,false);}catch{}
  $('#page').innerHTML=`
    <div class="page-head"><div><span class="eyebrow">Fotos</span><h1>Fotos no computador</h1><p class="muted">Copia as fotos armazenadas no Rota Próxima para a pasta local Rota - PEV. Não instala nenhum programa no Windows.</p></div></div>
    ${!supported?'<div class="warning"><strong>Navegador incompatível.</strong><br>Abra o Rota Próxima pelo Chrome ou Edge atualizado e usando HTTPS.</div>':''}
    <div class="card sync-card">
      <div class="sync-grid">
        <div><span>Pasta autorizada</span><strong id="photoSyncFolder">${esc(state.localSyncHandle?.name||'Nenhuma pasta autorizada')}</strong></div>
        <div><span>Última sincronização</span><strong id="photoSyncLast">${state.localSyncLast?esc(state.localSyncLast.toLocaleString('pt-BR')):'Ainda não executada'}</strong></div>
        <div><span>Automático</span><strong>${granted?'Ativo enquanto o Rota estiver aberto':'Aguardando autorização'}</strong></div>
      </div>
      <div class="info" style="margin-top:14px"><strong>Destino no seu computador</strong><br>C:\\Users\\balan\\OneDrive - GGJ Consultoria\\Área de Trabalho\\Rota - PEV<br><span class="muted">Por segurança, o navegador não revela o caminho completo. Na seleção, escolha exatamente essa pasta.</span></div>
      <div class="sync-actions">
        <button id="choosePhotoSyncFolder" class="btn secondary" ${supported?'':'disabled'}>${state.localSyncHandle?'Trocar pasta':'Selecionar pasta Rota - PEV'}</button>
        <button id="authorizePhotoSyncFolder" class="btn secondary" ${state.localSyncHandle&&!granted?'':'disabled'}>Autorizar pasta</button>
        <button id="syncPhotosNow" class="btn primary" ${supported?'':'disabled'}>Sincronizar agora</button>
      </div>
      <div id="photoSyncLiveStatus" class="sync-status">${granted?'Pronto para sincronizar.':'Selecione/autorize a pasta para começar.'}</div>
      <div class="muted" style="margin-top:12px">A sincronização automática ocorre a cada 2 minutos enquanto esta página do Rota Próxima estiver aberta no navegador. Se o computador ou navegador estiver fechado, as fotos permanecem seguras no Supabase e serão copiadas depois.</div>
    </div>`;
  if($('#choosePhotoSyncFolder'))$('#choosePhotoSyncFolder').onclick=async()=>{try{const h=await chooseLocalSyncFolder();if(h)renderPhotoSync();}catch(e){toast(e.message,'error')}};
  if($('#authorizePhotoSyncFolder'))$('#authorizePhotoSyncFolder').onclick=async()=>{try{if(!state.localSyncHandle)return;const ok=await handlePermission(state.localSyncHandle,true);if(!ok)throw new Error('Permissão não concedida.');startLocalSyncTimer();renderPhotoSync();}catch(e){toast(e.message,'error')}};
  if($('#syncPhotosNow'))$('#syncPhotosNow').onclick=async()=>{try{await syncEvidenceFiles(true);}catch(e){toast(e.message,'error');updatePhotoSyncStatus(e.message,'warn')}};
}
